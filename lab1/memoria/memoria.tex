...
\textbf{V1}: tweet original + text\_clean básico (lowercase, elimina URLs/menciones/hashtags).

\textbf{V2}: Mejoras en text\_clean: eliminación de stopwords, preservación de negaciones \{no, nunca, jamás, nada, nadie, tampoco, ni, sin\} con marcado NEG\_, normalización de acentos/elongaciones.

\subsection{Modelos evaluados}

\textbf{Clásicos (TF-IDF)}: Logistic Regression, LinearSVC, Random Forest, Gradient Boosting, Naive Bayes, Stacking.

\textbf{Modelos de embeddings}: F2LLM-4B (4B params, español) y KaLM (multilingüe).


\textbf{LLMs con LoRA}: Ministral-3-8B-Instruct (8B params).
...
\textbf{Observaciones}: F2LLM-4B (tweet) V2 mejora +0.001 puntos de F1-Macro. KaLM V2 empeora. Ministral-3-8B FT V2 falla drásticamente (F1=0.4029).
