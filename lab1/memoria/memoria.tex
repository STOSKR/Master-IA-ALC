...
\title{%
    \Large\textbf{Memoria Técnica}\\[0.3cm]
    \huge\textbf{Clasificación de Sexismo en Redes Sociales}\\[0.2cm]
    \large{EXIST 2025 - Task 1}
}
\author{Master en Inteligencia Artificial}
\date{\today}
...
Se implementaron dos estrategias de preprocesamiento:
...
Se experimentó con tres familias de modelos:
...
\midrule
F2LLM-4B (tweet) & \textbf{0.8604} & \textbf{0.8294} & 0.8642 & \textbf{0.8464} \\
...
\midrule
Ministral-3-8B (ZS) & - & - & - & - \\
Ministral-3-8B (FT) & \multicolumn{4}{c}{\textit{En entrenamiento}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Modelos de Lenguaje}: Los modelos transformer fine-tuned superan significativamente a los modelos clásicos, con mejoras de hasta \textbf{+10.5 puntos} en F1-Score (F2LLM-4B vs Stacking).
...
\begin{enumerate}
    ...
    \item \textbf{Mayor Precision}: F2LLM-4B con tweet original (0.8294)
...
\begin{itemize}
    \item Completar evaluación de Ministral-3-8B post fine-tuning
