...
#SBATCH --mem=2G
#SBATCH -o logs/launcher_%j.log

# Crear directorios necesarios si no existen
mkdir -p logs
mkdir -p entregables

# Lista de notebooks a ejecutar en paralelo (separados por espacios)
...
# Lanzar un job por cada notebook
for NOTEBOOK in $NOTEBOOKS; do
    echo "Submitting job for: $NOTEBOOK"
    
    sbatch <<EOF
#!/bin/bash
#SBATCH -p long
#SBATCH --cpus-per-task=8
#SBATCH --job-name=${NOTEBOOK%.ipynb}
#SBATCH --mem=32G
#SBATCH --gres=shard:1
#SBATCH -o logs/${NOTEBOOK%.ipynb}_%j.log
...
echo "Use 'squeue' to check job status"
echo "=========================================="